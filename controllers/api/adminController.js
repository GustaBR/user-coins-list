...
const deleteCompletion = async (req, res) => {
    try {
        ...
        if (!req.params.id) {
            return res.status(400).json({ message: "Missing completion."});
        }
        
        const completion = await Completion.deleteOne( { _id: req.params.id } );

        await Completion.deleteOne(completion);
